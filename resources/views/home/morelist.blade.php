    ...
    <div class="middle">            <!-- 中部内容 -->
      ...
      <div class="navigation">
        <ol class="wp-paginate">
          ...
          <?php if ($page>3): ?>
            <li><a href="{{url('morelist')}}?id={{$id}}&&page=1" title="1" class="page">1</a></li>
          <?php endif; ?>
          <?php for ($i=$page-2; $i < $page&&$i>0; $i++):  ?>
            <li><a href="{{url('morelist')}}?id={{$id}}page={{@$i}}" title="{{@$i}}" class="page">{{@$i}}</a></li>
          <?php endfor; ?>
          <li><span class="page current">{{$page}}</span></li>

          <li><span class="gap">...</span></li>

          <li><a href="{{url('morelist')}}?id={{$id}}&&page={{$page+1}}" class="next">&gt;</a></li>
        </ol>
      </div>
      <?php else: ?>
      <div class="middle">
