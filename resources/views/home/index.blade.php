...
<style>
    body{background: url('http://togethergo.me/resources/file/image/home/beijing.jpg') repeat;background-size: 25%;margin-bottom: 20px;}
    ...
    div.second{background-color: ;min-height: 1000px;width: 1080px;margin: auto;margin-top: 50px;}
    ...
    div.middle-top{position: static;text-align: center;filter: blur(0px);background: url('http://togethergo.me/resources/file/image/home/xiaobeijing1.jpg');background-size: contain;color: white;}
    ...
    div.right{background-color: ;position: absolute;width: 180px;margin-left: 905px;margin-top: 20px;}
  ...
  <div class="second">
    ...
    <div class="left">        <!-- 左侧导航栏 -->
      ...
      <div class="left-bar">
          <ul class="left-bar">
            <li>
            <ul><a href="#" class="left-bar">首页</a></ul>
    ...
    <div class="right">             <!-- 右侧导航栏 -->
      <div class="right-bar">
        <ul>
          <li>
            <ul>
              <span class="fangwen">访问：{{$usermsg->view}}</span>
            </ul>
            <ul>
              <span class="fangwen">文章：{{$usermsg->original+$usermsg->reprint}}</span>
            </ul>
            <ul>
              <span class="fangwen">转载：{{$usermsg->reprint}}</span>
            </ul>
          </li>
        </ul>
      ...
      <div class="right-bar">
        <ul class="left-bar">
          <p class="smheader"><span>热门文章</span></p>
          <?php foreach ($essay['hot'] as $value): ?>
            <li>  <a href="{{url('essay')}}?id={{$value->art_id}}" class="left-bar-fenlei"><span class="msgtome">{{$value->art_title}}</span></a></li>
          <?php endforeach; ?>
          <!-- <li class="smtitle"><span class="left-bar">钢铁是怎样炼成的</span></li>
          <li class="smtitle"><span class="left-bar">钢铁是怎样炼成的</span></li>
          <li class="smtitle"><span class="left-bar">钢铁是怎样炼成的</span></li> -->
        </ul>
      </div>
      <div class="right-bar">
        <ul class="left-bar">
          <li><p class="smheader">最近动态</p></li>
          <?php foreach ($essay['new'] as $value): ?>
            <li>  <a href="{{url('essay')}}?id={{$value->art_id}}" class="left-bar-fenlei"><span class="msgtome">{{$value->art_title}}</span></a></li>
          <?php endforeach; ?>
    ...
    <div class="middle">            <!-- 中部内容 -->
            ...
            <div class="content-img"><a href="{{url('essay')}}?id={{$value->art_id}}" >
              ...
              for ($i=1; $i  ; $i++) {
                foreach ($img as $key) {$arr1=preg_match('/src=[\"|\'][^\",^\']*[\'|\"]/',$key,$arr2);
                  ...
                  echo 'src="http://togethergo.me/resources/file/image/home/timg.jpg"';
                  end:
                  break;
              }?> alt="" width="550px">
              </a></div>
      ...
      <div class="navigation">
        <ol class="wp-paginate">
          ...
          <?php if ($msg['page']>3): ?>
            <li><a href="{{url('')}}?page=1" title="1" class="page">1</a></li>
          <?php endif; ?>
          <?php for ($i=$msg['page']-2; $i < $msg['page']&&$i>0; $i++):  ?>
            <li><a href="{{url('')}}?page={{@$i}}" title="{{@$i}}" class="page">{{@$i}}</a></li>
          <?php endfor; ?>
          <li><span class="page current">{{$msg['page']}}</span></li>
          <?php for ($i=$msg['page']+1; $i < $msg['page']+3&&$i<=$msg['pagenum']; $i++):  ?>

            <li><a href="{{url('')}}?page={{@$i}}" title="{{@$i}}" class="page">{{@$i}}</a></li>
          <?php endfor; ?>
          <li><span class="gap">...</span></li>
          <li><a href="{{url('')}}?page={{$msg['pagenum']}}" title="{{$msg['pagenum']}}" class="page">{{$msg['pagenum']}}</a></li>
